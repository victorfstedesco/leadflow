<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto">
        <div class="mb-10">
            ...
            <h1 class="section-title">Nova postagem</h1>
            <p class="text-gray-500 mt-2">Crie um novo conteúdo para <strong>{{ $client->name }}</strong>.</p>
        </div>

        <form method="POST" action="{{ route('posts.store', $client) }}" class="grid md:grid-cols-3 gap-8">
            @csrf
                ...
                <div class="card p-8 border-gray-100 shadow-sm relative overflow-hidden">
                    ...
                    <div class="space-y-6 relative z-10">
                        <div>
                            ...
                            <input type="text" name="title" required class="input font-semibold" value="{{ old('title', request('suggested_title')) }}" placeholder="Ex: 5 dicas para cuidar da pele no verão">
                        ...
                        <div>
                            ...
                            <textarea name="copy" rows="6" class="input" placeholder="Texto que acompanhará a publicação nas redes sociais...">{{ old('copy') }}</textarea>
                        ...
                        <div class="pt-2">
                            ...
                            <select name="campaign_id" class="input">
                                ...
                                @foreach ($campaigns as $campaign)
                                    <option value="{{ $campaign->id }}" @selected((int) old('campaign_id') === $campaign->id)>{{ $campaign->name }}</option>
                                @endforeach
                            </select>
